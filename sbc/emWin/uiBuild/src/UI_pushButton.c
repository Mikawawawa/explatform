...
static void _cbManagerButton(WM_MESSAGE *pMsg)
{
	WM_HWIN hButton;
	hButton = pMsg->hWin;
	switch (pMsg->MsgId)
	...
	case WM_PAINT:
		if (BUTTON_IsPressed(hButton))
		{
			GUI_SetBkColor(0x00016781);
			GUI_Clear();
			GUI_SetAlpha(0x20);
			GUI_DrawBitmap(&bmmanager, 0, 0);
			GUI_SetAlpha(0x00);
...
void pushButtonInitSettingDialog(WM_HWIN hWin)
{
	...
	hButton = WM_GetDialogItem(hWin, ID_BUTTON_6);
	BUTTON_SetText(hButton, "");
	WM_SetCallback(hButton, _cbManagerButton);
}
